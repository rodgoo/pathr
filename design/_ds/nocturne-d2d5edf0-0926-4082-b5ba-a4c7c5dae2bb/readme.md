...
Only `styles.css`, `readme.md` and `_ds_bundle.js` were imported into this repository — the files the canvas actually references. The `foundations/`, `components/` and `templates/` pages listed above live in the Claude Design project and are not vendored here. The implementation in `frontend/` links `styles.css` (copied to `frontend/src/styles/nocturne.css`) and follows this guide.
